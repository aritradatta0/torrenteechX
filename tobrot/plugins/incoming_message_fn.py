...
async def incoming_message_f(client, message):
    ...
    credit = await message.reply_text(
        f"Downloading <a href='tg://user?id={g_id}'></a>", parse_mode="html"
    )
...
async def incoming_youtube_dl_f(client, message):
    ...
    current_user_id = message.from_user.id
    credit = await message.reply_text(
        f"Downloading <a href='tg://user?id={current_user_id}'></a>",
        parse_mode="html",
    )
    i_m_sefg = await message.reply_text("processing...", quote=True)
    # LOGGER.info(message)
...
async def g_yt_playlist(client, message):
    ...
    if message.reply_to_message:
        ...
        await message.reply_text("No download source provided.", quote=True)
        return
    if "youtube.com/playlist" in url:
        i_m_sefg = await message.reply_text(
            f"Downloading <a href='tg://user?id={usr_id}'></a>",
            parse_mode="html",
        )
...
async def rename_tg_file(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("No download source provided!", quote=True)
        return
